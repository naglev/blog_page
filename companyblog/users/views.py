...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Thanks for registration!') #nem kell mindkettő, mert ez csak nagyon rövid ideig lesz léátható a redirect miatt
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and user.check_password(form.password.data):
            ...
            if next==None or not next[0]=='/':  #ha nem az van beírva pl. /account hanem önkényesen írtunk be valamit
